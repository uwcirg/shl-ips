...
<DatasetOffcanvas bind:isOpen={isOpen} {dataset}>
  <Button
    slot = "action-button-start"
    size="sm"
    outline
    color="secondary"
    on:click={() => { isOpen = false; updateDataset(dataset.collection) }}
  >
    <Icon name="arrow-repeat" /> Update
  </Button>
  <Button
    slot = "action-button-end"
    size="sm"
    outline
    color="danger"
    on:click={() => {
      ...
      <Col xs="12" sm="6" lg="4" style="">
        <DatasetView {dataset} {masterPatient}>
          <DropdownMenu slot="menu">
            <DropdownItem on:click={() => showDataset(dataset)}><div class="d-flex justify-content-between w-100">View <Icon name="chevron-right"/></div></DropdownItem>
          ...
          <Button slot="footer" class="d-flex justify-content-between align-items-center" color="secondary" outline on:click={() => showDataset(dataset)}>
