    ...
    <CardBody>
        {#if resource.medicationCodeableConcept}
            ...
            {:else}
                ...
                {resource.medicationCodeableConcept.display ?? ""} <span class="badge badge-secondary">{resource.medicationCodeableConcept.code ?? ""}</span>
            {/if}
        {/if}
        {resource.intent ? `Intent: ${resource.intent}` : ""}
        {resource.status ? `Status: ${resource.status}` : ""}
        {resource.dispenseRequest?.validityPeriod ? `Period: ${resource.dispenseRequest.validityPeriod}` : ""}
